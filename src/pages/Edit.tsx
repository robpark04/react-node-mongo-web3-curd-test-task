...
const Edit = () => {
  ...
  const handleUpdate = async (e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
    e.preventDefault();
    ...
    UseApis.update("/nfts/update/", Id, formData).then((res) => {
      if (res.Status) {
  ...
  useEffect(() => {
    UseApis.get<NFT>("/nfts/edit/", id).then((res) => {
      if (res.Status) {
        const data = res.Data;
        setId(data._id)
        setName(data.name);
